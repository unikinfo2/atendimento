inherited EMail_f: TEMail_f
  Left = 139
  Top = 113
  Width = 653
  Height = 479
  Caption = 'Emiss'#227'o de E-Mail'#39's'
  OldCreateOrder = True
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel [0]
    Left = 0
    Top = 377
    Width = 637
    Height = 44
    Align = alBottom
    TabOrder = 2
    object btnEnviar: TBitBtn
      Left = 233
      Top = 6
      Width = 87
      Height = 33
      Caption = '&Enviar'
      TabOrder = 0
      OnClick = btnEnviarClick
      Glyph.Data = {
        
        77BB3333333337333377333333333777337B3333333333333337}
      NumGlyphs = 2
    end
    object btnCancelar: TBitBtn
      Left = 324
      Top = 6
      Width = 87
      Height = 33
      Caption = '&Cancelar'
      TabOrder = 1
      OnClick = btnCancelarClick
      Glyph.Data = {
        
        50BB555555555555575F555555555555550B5555555555555575}
      NumGlyphs = 2
    end
  end
  object Panel2: TPanel [1]
    Left = 0
    Top = 203
    Width = 637
    Height = 174
    Align = alClient
    TabOrder = 1
    object memoTexto: TMemo
      Left = 1
      Top = 1
      Width = 635
      Height = 172
      Align = alClient
      TabOrder = 0
    end
  end
  object Panel3: TPanel [2]
    Left = 0
    Top = 0
    Width = 637
    Height = 203
    Align = alTop
    TabOrder = 0
    object Label1: TLabel
      Left = 4
      Top = 44
      Width = 89
      Height = 13
      Caption = 'Informe os Clientes'
    end
    object Label2: TLabel
      Left = 312
      Top = 44
      Width = 75
      Height = 13
      Caption = 'Com Copia para'
    end
    object Label3: TLabel
      Left = 312
      Top = 81
      Width = 38
      Height = 13
      Caption = 'Assunto'
    end
    object sbPesEmail: TSpeedButton
      Left = 83
      Top = 19
      Width = 23
      Height = 22
      Glyph.Data = {
        
        3000333333333333377733333333333333333333333333333333}
      NumGlyphs = 2
      OnClick = sbPesEmailClick
    end
    object Label10: TLabel
      Left = 7
      Top = 3
      Width = 29
      Height = 13
      Caption = 'E-Mail'
    end
    object Label4: TLabel
      Left = 293
      Top = 5
      Width = 344
      Height = 13
      Caption = 
        '* Os Clientes/Contatos cadastrados sem e-mail n'#227'o constar'#227'o nest' +
        'a lista.'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -8
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object clbCliente: TCheckListBox
      Left = 4
      Top = 58
      Width = 301
      Height = 138
      ItemHeight = 13
      TabOrder = 1
    end
    object edtComCopia: TEdit
      Left = 312
      Top = 58
      Width = 329
      Height = 21
      TabOrder = 2
    end
    object edtAssunto: TEdit
      Left = 312
      Top = 95
      Width = 329
      Height = 21
      TabOrder = 3
    end
    object memoArquivo: TMemo
      Left = 312
      Top = 119
      Width = 233
      Height = 77
      ReadOnly = True
      ScrollBars = ssHorizontal
      TabOrder = 4
      WordWrap = False
    end
    object btnArquivo: TBitBtn
      Left = 553
      Top = 134
      Width = 85
      Height = 41
      Caption = '&Anexar Arquivos'
      TabOrder = 5
      OnClick = btnArquivoClick
      Glyph.Data = {
        
        005555555775FFFF77F555555570000000555555555777777755}
      Layout = blGlyphTop
      NumGlyphs = 2
    end
    object medEmail: TMaskEdit
      Left = 7
      Top = 19
      Width = 73
      Height = 21
      EditMask = '#########;0; '
      MaxLength = 9
      TabOrder = 0
      OnExit = medEmailExit
    end
  end
  inherited submenu: TMainMenu
    Left = 572
    Top = 252
  end
  object dsEmail: TDataSource [4]
    AutoEdit = False
    DataSet = qryEmail
    Left = 272
    Top = 160
  end
  object qryEmail: TQuery [5]
    DatabaseName = 'db'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM GC_EMAIL WHERE COD_EMAIL = :COD_EMAIL')
    Left = 304
    Top = 160
    ParamData = <
      item
        DataType = ftInteger
        Name = 'COD_EMAIL'
        ParamType = ptUnknown
      end>
  end
  object qryAux: TQuery [6]
    DatabaseName = 'db'
    Left = 384
    Top = 160
  end
  object Arquivo: TOpenDialog [7]
    Filter = '*.*|*.*'
    Title = 'Atachamento de Arquivos'
    Left = 224
    Top = 193
  end
  object SakSMTP1: TSakSMTP [8]
    Port = '25'
    EncodeProgressStep = 10
    SendProgressStep = 10
    OnQuit = SakSMTP1Quit
    OnBeforeSend = SakSMTP1BeforeSend
    OnSendProgress = SakSMTP1SendProgress
    Left = 112
    Top = 120
  end
  object SakMsg1: TSakMsg [9]
    Left = 112
    Top = 152
  end
  object qryEmailCli: TQuery [10]
    DatabaseName = 'db'
    RequestLive = True
    SQL.Strings = (
      'SELECT * FROM GC_EMAIL_CLI WHERE COD_EMAIL = :COD_EMAIL')
    Left = 272
    Top = 200
    ParamData = <
      item
        DataType = ftInteger
        Name = 'COD_EMAIL'
        ParamType = ptUnknown
      end>
  end
end
